	...
	Set<Unit> handlerUnits = new ArraySet(body.getTraps().size());
		...
		handlerUnits.add(trap.getHandlerUnit());
                if(trap.getBeginUnit() != trap.getEndUnit()) {
                    emit(".catch " + slashify(trap.getException().getName()) + " from__reserved__jasmin " +
                        unitToLabel.get(trap.getBeginUnit()) + " to__reserved__jasmin " + unitToLabel.get(trap.getEndUnit()) +
                        " using__reserved__jasmin " + unitToLabel.get(trap.getHandlerUnit()));
		}
            }
        }
