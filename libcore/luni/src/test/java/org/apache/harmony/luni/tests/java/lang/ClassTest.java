...
public class ClassTest extends junit.framework.TestCase {
    ...
    /*
     * TODO(Fixed) Harmony seems to assume that packages are always defined.
     * This isn't (currently) true for our VM and it also isn't required by
     * the Java spec. Thus we get the package name a different way.
     */
    public static final String FILENAME = getPackageName(); // ClassTest.class.getPackage().getName().replace('.', '/')+"/test#.properties";

    static String getPackageName() {
        String s = ClassTest.class.getName();
        int dot = s.lastIndexOf('.');
        return s.substring(0, dot).replace('.', '/')+"/test#.properties";
    }
